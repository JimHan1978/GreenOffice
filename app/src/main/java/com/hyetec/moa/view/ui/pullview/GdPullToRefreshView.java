...
/**
 * 名称：GbPullToRefreshView.java
 * 描述：下拉刷新和加载更多的View.
 */
public class GdPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private GdListViewHeader mHeaderView;
	
	/** footer view. */
	private GdListViewFooter mFooterView;
	 ...
	 */
	public GdPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public GdPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void addHeaderView() {
        mHeaderView = new GdListViewHeader(mContext);
	...
	private void addFooterView() {
        ...
        mFooterView = new GdListViewFooter(mContext);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != GdListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(GdListViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(GdListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != GdListViewFooter.STATE_LOADING) {
			mFooterView.setState(GdListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(GdListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(GdListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(GdListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(GdListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(GdListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(GdListViewFooter.STATE_READY);
		}
     ...
     */
    public GdListViewHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public GdListViewFooter getFooterView() {
        return mFooterView;
    }
