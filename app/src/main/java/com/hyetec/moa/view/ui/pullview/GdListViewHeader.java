...
/**
 * 名称：GbListViewHeader.java
 * 描述：下拉刷新的Header View类.
 */
public class GdListViewHeader extends LinearLayout {
	 ...
	 */
	public GdListViewHeader(Context context) {
		super(context);
		initView(context);
	}
	 ...
	 */
	public GdListViewHeader(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView(context);
	}
	...
	private void initView(Context context) {
		...
		GdViewUtil.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage = BitmapFactory.decodeResource (getResources(),R.drawable.z_arrow_down);
		...
		layoutParamsWW.width = GdViewUtil.scale(mContext, 50);
		layoutParamsWW.height = GdViewUtil.scale(mContext, 50);
		...
		GdViewUtil.setPadding(headTextLayout,0, 0, 0, 0);
		...
		GdViewUtil.setTextSize(tipsTextview,30);
		GdViewUtil.setTextSize(headerTimeView,27);
		...
		layoutParamsWW3.rightMargin = GdViewUtil.scale(mContext, 10);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = GdDateUtil.getCurrentDate(GdDateUtil.dateFormatHMS);
