...
public class GdViewUtil {
    ...
    public static float dip2px(Context context, float dipValue) {
        DisplayMetrics mDisplayMetrics = GdAppUtil.getDisplayMetrics(context);
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		float scale = 1;
		try {
			float scaleWidth = (float) displayWidth / GdAppConfig.UI_WIDTH;
			float scaleHeight = (float) displayHeight / GdAppConfig.UI_HEIGHT;
	 ...
	 * 
	 * 描述：View树递归调用做适配.
	 * GdAppConfig.uiWidth = 1080;
	 * GdAppConfig.uiHeight = 700;
    ...
    public static void scaleContentView(ViewGroup contentView){
        GdViewUtil.scaleView(contentView);
    ...
    */
    public static void setViewSize(View view, int widthPixels, int heightPixels){
        ...
        if(params == null){
            GdLogUtil.e(GdViewUtil.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
